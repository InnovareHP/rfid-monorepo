...
# ── Database ───────────────────────────────────────────────
# No inbound CIDR rules at all. Postgres is reachable only from the ECS tasks
# and from the SSM forwarder, both by security group reference.
resource "aws_security_group" "db" {
  name        = "${var.name_prefix}-db"
  description = "Postgres 5432 from ECS tasks and the SSM forwarder"
  vpc_id      = var.vpc_id
...
# The forwarder has zero inbound rules of its own. SSM opens an outbound
# connection to the agent, so nothing needs to reach it.
resource "aws_security_group" "db_tunnel" {
  name        = "${var.name_prefix}-db-tunnel"
  description = "SSM port-forward host. Egress only."
  vpc_id      = var.vpc_id
...
resource "aws_security_group_rule" "db_from_tunnel" {
  type                     = "ingress"
  from_port                = 5432
  to_port                  = 5432
  protocol                 = "tcp"
  source_security_group_id = aws_security_group.db_tunnel.id
  security_group_id        = aws_security_group.db.id
  description              = "SSM forwarder to Postgres"
}
