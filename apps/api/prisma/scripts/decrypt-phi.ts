...
// Reverses encrypt-phi.ts: rewrites enc:v1: values back to plaintext.
// Requires the same ENCRYPTION_KEY the rows were encrypted with.
...
async function run() {
  for (const spec of SPECS) {
    ...
    console.log(`[${spec.label}] decrypted ${touched}/${rows.length}`);
  }
}
