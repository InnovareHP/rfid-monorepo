...
export const CreateSenderSchema = z
  .object({
    ...
    // The subdomain under the domain we run, e.g. "acme" -> acme.mail.<ours>.
    subdomain: z
      .string()
      .trim()
      .toLowerCase()
      .regex(/^(?!-)[a-z0-9-]{1,40}(?<!-)$/, "Use letters, numbers, dashes")
      .optional(),
  ...
  .superRefine((values, ctx) => {
    if (values.kind === "MANAGED_DOMAIN" && !values.subdomain) {
      ctx.addIssue({
        code: "custom",
        path: ["subdomain"],
